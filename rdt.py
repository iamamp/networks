...
Bob and Alice communicate using pydispatch.
The code demonstrates an understanding of what happens when packets are dropped (purposely
in this case), timeouts occur - checking the ACK and retransmitting if required.
Hash values acting as checksums are used to verify content of messages.

The chance of a string of successful transmissions is intentionally kept low so as to
better demonstrate the failure cases.

note - negative ACK for packet with seq0 will be ACK1
